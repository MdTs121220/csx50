...
-- step 1 -- filter from table crime
SELECT description FROM crime_scene_reports
WHERE year = 2021 AND month = 7 AND day = 28 AND street = 'Humphrey Street';

-- step 2 -- find transkrip about the incidents
SELECT name, transcript FROM interviews
WHERE year = 2021 AND month = 7 AND day = 28;

-- step 3 -- found two people have the same name, filtering that name again
SELECT name FROM people
WHERE name = 'Eugene';

--step 4 -- try to filter the transcript about bakery from witness to give interviews day of incidents
SELECT name, transcript FROM interviews
WHERE year = 2021 AND month = 7 AND day = 28 AND transcript LIKE '%bakery%' ORDER BY name;

--step 5 -- try to check their interview -- start from eugene clues
SELECT account_number, amount FROM atm_transactions
WHERE year = 2021 AND month = 7 AND day = 28 AND atm_location = 'Leggett Street' AND transaction_type = 'withdraw';

--step 6 -- find corresponding account numbers from step 5
...
-- step 7 -- try to check raymond clues -> Finding the flights on July 29 from Fiftyville airport
...
-- step 8 -> Found First flight comes out to be at 8:20 to LaGuardia Airport in New York City (Flight id- 36). maybe found the place where the thief (check the passengers)
...
--- step 9 -> try to check the phone call
...
--- step 10 -> check names of the call and name receive
SELECT name, phone_calls.duration FROM people
JOIN phone_calls ON people.phone_number = phone_calls.receiver
WHERE phone_calls.year = 2021 AND phone_calls.month = 7 AND phone_calls.day = 28
AND phone_calls.duration <= 60 ORDER BY phone_calls.duration;

--- step 11 -> check clues from ruth
...
AND bakery_security_logs.minute >= 15 AND bakery_security_logs.minute <= 25 ORDER BY bakery_security_logs.minute;